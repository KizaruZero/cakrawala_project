...
const DELETABLE_STAGES = ["New", "Cancelled"];
const BLOCK_ARCHIVE_STAGES = ["New", "In Progress", "On Hold"];

const POLICY = {
    "helpdesk.ticket": {
        fields: ["active", "stage_name"],
        canDelete: (data) =>
            data.active === false || DELETABLE_STAGES.includes(data.stage_name),
        canArchive: (data) =>
            data.active === false || !BLOCK_ARCHIVE_STAGES.includes(data.stage_name),
    },
};
