...
class PurchaseOrderVeselMaster(models.Model):
    _name = "purchase.order.vesel.master"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Purchase Order Vesel"
...
class PurchaseOrderTypeMaster(models.Model):
    _name = "purchase.order.type.master"
    _inherit = ['mail.thread', 'mail.activity.mixin']
...
class PurchaseOrderApprovalConfigMaster(models.Model):
    _name = "purchase.order.approval.config.master"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    ...
    purchase_order_type_id = fields.Many2one('purchase.order.type.master', string='PO Type', required=True, tracking=True, check_company=True,
    ...
    def button_confirm(self):
        for record in self:
            if record.state not in ['draft']:
                continue
            name = f"{record.company_id.company_code}/{record.purchase_order_type_id.name}/{record.department_id.name}/{record.sequence}"
            ...
            if dup:
                raise ValidationError(
                    _("PO Approval with the same name already exist '%s', please create another one or modify existing data!!") % name
                )
