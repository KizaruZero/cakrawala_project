...
class FleetContractConfirmWizard(models.TransientModel):
    ...
    def action_confirm(self):
        ...
        vals = {
            'name': f"{contract.license_plate} - {contract.vehicle_id.asset_number}",
            'asset_number': contract.vehicle_id.asset_number,
        ...
        if existing:
            existing.write(vals)
            contract.vehicle_id.analytic_account_id = existing.id
        else:
            analytic = Analytic.create(vals)
            contract.vehicle_id.analytic_account_id = analytic.id
