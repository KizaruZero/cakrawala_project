...
class SaleOrder(models.Model):
    ...
    contract_file = fields.Binary(string="Contract Document", attachment=True)
    ...
    def action_create_contract(self):
        ...
        try:
            ...
            raise UserError(_("Python library 'docxtpl' is not installed. Please install it using 'pip install docxtpl'."))
        ...
        customer_address_lines = [line.strip() for line in customer_address.split('\n') if line.strip()]
        
        # Pad address lines to ensure we have at least 3
        while len(customer_address_lines) < 3:
            customer_address_lines.append("")
            
        customer_address_line1 = customer_address_lines[0]
        customer_address_line2 = customer_address_lines[1] if len(customer_address_lines) > 1 else ""
        customer_address_line3 = customer_address_lines[2] if len(customer_address_lines) > 2 else ""

        customer_phone = self.partner_id.phone or "[Telepon Customer]"
        customer_fax = getattr(self.partner_id, 'fax', '') or "[Fax Customer]"
        ...
        if today_dt:
            ...
            tanggal_mulai = f"{today_dt.day:02d} {bulan_str} {today_dt.year}"
        ...
        context = {
            ...
            'customer_pic_title': customer_pic_title,
        ...
        # Load template
        try:
            template_path = file_path('x_sale_purchase_custom/static/src/templates/contract_template.docx')
        except FileNotFoundError:
            raise UserError(_("Contract template not found at x_sale_purchase_custom/static/src/templates/contract_template.docx"))
        ...
        # Save to attachment field
        self.contract_filename = f"Kontrak Sewa - {customer_name}.docx"
        self.contract_file = base64.b64encode(file_content)
        
        # Return action to download file or refresh view
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
                'title': _('Contract Generated'),
                'message': _('The contract document has been created successfully.'),
                'sticky': False,
                'type': 'success',
                'next': {
                    'type': 'ir.actions.client',
