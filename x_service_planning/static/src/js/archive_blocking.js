...
const isUntouched = (data) => data.state === "active" && !data.has_related_document;

const POLICY = {
    "service.planning": {
        fields: ["state", "has_related_document"],
        canDelete: (data) =>
            ["active", "cancelled"].includes(data.state) && !data.has_related_document,
        canArchive: (data) => !isUntouched(data),
    },
};
