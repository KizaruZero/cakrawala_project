...
class PurchaseOrderAdvancePaymentLine(models.TransientModel):
    _name = 'purchase.order.advance.payment.line'
    _description = 'Purchase Order Advance Payment Wizard Line'

    wizard_id = fields.Many2one('purchase.order.advance.payment', string='Wizard')
...
class PurchaseOrderAdvancePayment(models.TransientModel):
    ...
    def _create_bill(self, order, so_line, amount):
        ...
        # Add extra manual lines from wizard
    ...
    def _prepare_down_payment_product_values(self):
        return {
            ...
            'supplier_taxes_id': [Command.set(self.deposit_taxes_ids.ids)],
        }
